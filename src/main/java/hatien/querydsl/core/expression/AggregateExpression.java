...
/**
 * Base class for all SQL aggregate function expressions (COUNT, SUM, AVG, MIN,
 * MAX). Aggregate expressions operate on groups of rows and return a single
 * value.
 *
...
public abstract class AggregateExpression<T> implements Expression<T> {
	...
	/**
	 * Constructs a new AggregateExpression with the specified type, aggregate
	 * operation, and target expression.
	 *
	 * @param type          the return type of this aggregate function
	 * @param aggregateType the type of aggregate operation
	 * @param expression    the expression to aggregate over (can be null for
	 *                      COUNT(*))
	 */
	...
	/**
	 * Returns the expression being aggregated over. May be null for operations like
	 * COUNT(*).
	 *
