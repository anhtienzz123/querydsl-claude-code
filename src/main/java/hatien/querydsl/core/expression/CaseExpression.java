...
/**
 * Expression representing a SQL CASE WHEN construct.
 * Supports multiple WHEN clauses and an optional ELSE clause.
 * 
 * <p>Example usage:
 * <pre>
 * CaseExpression&lt;String&gt; caseExpr = CaseExpression.&lt;String&gt;builder(String.class)
...
public class CaseExpression<T> implements Expression<T> {
    ...
    public static class CaseBuilder<T> {
         ...
         * In SQL, this means NULL will be returned when no WHEN conditions match.
         *
         * @return the completed CaseExpression
         */
    ...
    private static class ConstantExpression<T> implements Expression<T> {
        ...
        @Override
        public <R> R accept(ExpressionVisitor<R> visitor) {
            // For constant values, we don't need visitor pattern - just return the string representation
