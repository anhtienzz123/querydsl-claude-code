...
/**
 * An executable implementation of the Query interface that can execute SQL
 * queries using JDBC. Extends QueryBuilder functionality with actual database
...
public class ExecutableQueryBuilder<T> implements Query<T> {
	...
	/**
	 * Creates a new ExecutableQueryBuilder with the specified executor and result
	 * type.
	 *
	...
	private String toParameterizedSQL() {
		...
		if (!selectList.isEmpty()) {
			sql.append("SELECT ");
			sql.append(selectList.stream().map(expr -> expr.accept(parameterizedSqlVisitor))
		...
		if (!groupByList.isEmpty()) {
			sql.append(" GROUP BY ");
			sql.append(groupByList.stream().map(expr -> expr.accept(parameterizedSqlVisitor))
					.collect(Collectors.joining(", ")));
		}
		...
		if (!havingList.isEmpty()) {
			sql.append(" HAVING ");
			sql.append(havingList.stream().map(predicate -> predicate.accept(parameterizedSqlVisitor))
					.collect(Collectors.joining(" AND ")));
		}
		...
		if (!orderByList.isEmpty()) {
			sql.append(" ORDER BY ");
			sql.append(orderByList.stream().map(expr -> expr.accept(parameterizedSqlVisitor))
					.collect(Collectors.joining(", ")));
		}
