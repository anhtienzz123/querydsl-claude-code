...
/**
 * Factory class for creating executable Query instances with JDBC database integration.
 * Provides convenient methods to start building queries that can execute against an actual database.
 */
public class ExecutableQueryFactory {
    ...
    /**
     * Creates a new ExecutableQueryFactory with the specified database configuration.
     *
    ...
    /**
     * Creates a new executable query starting with a SELECT clause for a single expression.
     *
    ...
    /**
     * Creates a new executable query starting with a SELECT clause for multiple expressions.
     * This method returns a Query<Object[]> since multiple columns are selected.
     *
     * @param expressions the expressions to select
     * @return a new executable Query instance with the SELECT clause configured for multiple columns
     */
    ...
    /**
     * Creates a new executable query starting with both SELECT and FROM clauses using the same expression.
     ...
     * @return a new executable Query instance with both SELECT and FROM clauses configured
     */
     ...
     * @return a new executable InsertQuery instance with the target entity configured
     */
     ...
     * @param <T> the type of the entity to update
     * @param entity the target entity path
     * @return a new executable UpdateQuery instance with the target entity configured
     */
     ...
     * @param <T> the type of the entity to delete
     * @param entity the target entity path
     * @return a new executable DeleteQuery instance with the target entity configured
     */
     ...
     * The transaction is automatically committed if the block completes successfully,
    ...
    public void inTransaction(TransactionBlock block) throws SQLException {
        try (Transaction transaction = beginTransaction()) {
            hatien.querydsl.core.database.TransactionalQueryExecutor txExecutor = 
    ...
    /**
     * Determines the result type for an expression.
