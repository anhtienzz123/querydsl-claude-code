 ...
 * 
 * This class shows how to use the ExecutableQueryFactory to perform actual
 * database operations. Note: These examples require a JDBC driver (like H2,
 * MySQL, PostgreSQL) to be available on the classpath.
 */
public class JdbcExecutionExamples {
	...
	private static void demonstrateQueryExecution() {
		 ...
		 * 
		 * try { ExecutableQueryFactory queryFactory =
		 * ExecutableQueryFactory.createH2InMemory("demodb");
		 * 
		 * // Create tables (this would normally be done with a schema migration tool)
		 * // setupDemoTables(queryFactory);
		 * 
		 * // INSERT example long insertCount = queryFactory
		 * .insertInto(user.getEntityPath()) .set(user.firstName, "John")
		 * .set(user.lastName, "Doe") .set(user.email, "john.doe@example.com")
		 * .set(user.age, 30) .set(user.city, "New York") .execute();
		 * 
		 * System.out.println("✓ Inserted " + insertCount + " user record");
		 * 
		 * // SELECT example List<User> users = queryFactory
		 * .selectFrom(user.getEntityPath()) .where(user.age.gt(25)) .fetch();
		 * 
		 * System.out.println("✓ Found " + users.size() + " users over 25");
		 * 
		 * // UPDATE example long updateCount = queryFactory
		 * .update(user.getEntityPath()) .set(user.email,
		 * "john.doe.updated@example.com") .where(user.firstName.eq("John")) .execute();
		 * 
		 * System.out.println("✓ Updated " + updateCount + " user records");
		 * 
		 * // COUNT example long userCount = queryFactory
		 * .selectFrom(user.getEntityPath()) .where(user.city.eq("New York"))
		 * .fetchCount();
		 * 
		 ...
		 * queryFactory.close();
		 * 
		 * } catch (Exception e) { System.err.println("Database execution failed: " +
		 * e.getMessage()); }
		 */
	}
	...
	private static void demonstrateTransactionUsage() {
		 ...
		 * 
		 * try { ExecutableQueryFactory queryFactory =
		 * ExecutableQueryFactory.createH2InMemory("txdemo");
		 * 
		 * // Automatic transaction example queryFactory.inTransaction(txFactory -> {
		 * txFactory.insertInto(user.getEntityPath()) .set(user.firstName, "Alice")
		 * .set(user.lastName, "Johnson") .set(user.age, 28) .execute();
		 * 
		 * txFactory.insertInto(user.getEntityPath()) .set(user.firstName, "Bob")
		 * .set(user.lastName, "Smith") .set(user.age, 32) .execute(); });
		 * 
		 * System.out.println("✓ Transaction completed successfully");
		 * 
		 * // Manual transaction example try (Transaction transaction =
		 * queryFactory.beginTransaction()) { ExecutableQueryFactory txFactory = new
		 * ExecutableQueryFactory( new TransactionalQueryExecutorWrapper( new
		 * TransactionalQueryExecutor(transaction) ) );
		 * 
		 * txFactory.insertInto(user.getEntityPath()) .set(user.firstName, "Charlie")
		 * .set(user.lastName, "Brown") .set(user.age, 35) .execute();
		 * 
		 * transaction.commit(); }
		 * 
		 ...
		 * queryFactory.close();
		 * 
		 * } catch (Exception e) { System.err.println("Transaction demo failed: " +
		 * e.getMessage()); }
		 */
	}
}