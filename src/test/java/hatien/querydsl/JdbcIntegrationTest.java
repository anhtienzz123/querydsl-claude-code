...
public class JdbcIntegrationTest {
	...
	private static void testInsertOperations(ExecutableQueryFactory queryFactory) {
		...
		// Insert users
		long insertCount1 = queryFactory.insertInto(user.getEntityPath()).set(user.firstName, "John")
				.set(user.lastName, "Doe").set(user.email, "john.doe@example.com").set(user.age, 30)
				.set(user.city, "New York").execute();

		System.out.println("✓ Inserted " + insertCount1 + " user record");

		long insertCount2 = queryFactory.insertInto(user.getEntityPath()).set(user.firstName, "Jane")
				.set(user.lastName, "Smith").set(user.email, "jane.smith@example.com").set(user.age, 25)
				.set(user.city, "Boston").execute();

		System.out.println("✓ Inserted " + insertCount2 + " user record");

		// Insert products
		long insertCount3 = queryFactory.insertInto(product.getEntityPath()).set(product.name, "Gaming Laptop")
				...
				.set(product.stockQuantity, 5).execute();
	...
	private static void testSelectOperations(ExecutableQueryFactory queryFactory) {
		...
		// Select all users
		List<User> allUsers = queryFactory.selectFrom(user.getEntityPath()).fetch();
		...
		List<User> youngUsers = queryFactory.selectFrom(user.getEntityPath()).where(user.age.lt(30)).fetch();

		System.out.println("✓ Found " + youngUsers.size() + " users under 30");

		// Select single user
		User singleUser = queryFactory.selectFrom(user.getEntityPath()).where(user.firstName.eq("John")).fetchFirst();

		System.out.println("✓ Single user lookup: "
				+ (singleUser != null ? singleUser.getFirstName() + " " + singleUser.getLastName() : "null"));

		// Count users
		long userCount = queryFactory.selectFrom(user.getEntityPath()).where(user.city.eq("New York")).fetchCount();

		System.out.println("✓ Users in New York: " + userCount);

		// Select products
		List<Product> products = queryFactory.selectFrom(product.getEntityPath())
				.where(product.price.gt(new BigDecimal("100"))).fetch();
	...
	private static void testUpdateOperations(ExecutableQueryFactory queryFactory) {
		...
		// Update user email
		long updateCount1 = queryFactory.update(user.getEntityPath()).set(user.email, "john.doe.updated@example.com")
				.where(user.firstName.eq("John")).execute();

		System.out.println("✓ Updated " + updateCount1 + " user email");

		// Update multiple fields
		long updateCount2 = queryFactory.update(user.getEntityPath()).set(user.city, "Los Angeles").set(user.age, 31)
				.where(user.firstName.eq("John"), user.lastName.eq("Doe")).execute();

		System.out.println("✓ Updated " + updateCount2 + " user records");

		// Update product price
		long updateCount3 = queryFactory.update(product.getEntityPath()).set(product.price, new BigDecimal("999.99"))
				.where(product.name.contains("Laptop")).execute();
	...
	private static void testDeleteOperations(ExecutableQueryFactory queryFactory) {
		...
		queryFactory.insertInto(user.getEntityPath()).set(user.firstName, "Test").set(user.lastName, "User")
				.set(user.email, "test@example.com").set(user.age, 99).set(user.city, "TestCity").execute();

		// Delete test user
		long deleteCount1 = queryFactory.deleteFrom(user.getEntityPath()).where(user.firstName.eq("Test")).execute();

		System.out.println("✓ Deleted " + deleteCount1 + " test user");

		// Insert and delete products
		queryFactory.insertInto(product.getEntityPath()).set(product.name, "Test Product")
				.set(product.price, new BigDecimal("1.00")).set(product.category, "Test").set(product.stockQuantity, 0)
				.execute();

		long deleteCount2 = queryFactory.deleteFrom(product.getEntityPath()).where(product.category.eq("Test"))
				.execute();
	...
	private static void testTransactions(ExecutableQueryFactory queryFactory) throws SQLException {
		...
		queryFactory.inTransaction(txFactory -> {
			txFactory.insertInto(user.getEntityPath()).set(user.firstName, "Alice").set(user.lastName, "Johnson")
					.set(user.email, "alice@example.com").set(user.age, 28).set(user.city, "Chicago").execute();

			txFactory.insertInto(user.getEntityPath()).set(user.firstName, "Bob").set(user.lastName, "Wilson")
					.set(user.email, "bob@example.com").set(user.age, 35).set(user.city, "Chicago").execute();
		});

		long chicagoUsers = queryFactory.selectFrom(user.getEntityPath()).where(user.city.eq("Chicago")).fetchCount();
		...
		try {
			queryFactory.inTransaction(txFactory -> {
				txFactory.insertInto(user.getEntityPath()).set(user.firstName, "Charlie").set(user.lastName, "Brown")
						.set(user.email, "charlie@example.com").set(user.age, 40).set(user.city, "Denver").execute();
		...
		long denverUsers = queryFactory.selectFrom(user.getEntityPath()).where(user.city.eq("Denver")).fetchCount();
		...
		try (Transaction transaction = queryFactory.beginTransaction()) {
			...
			txFactory.insertInto(user.getEntityPath()).set(user.firstName, "David").set(user.lastName, "Miller")
					.set(user.email, "david@example.com").set(user.age, 33).set(user.city, "Seattle").execute();

			transaction.commit();
		}

		long seattleUsers = queryFactory.selectFrom(user.getEntityPath()).where(user.city.eq("Seattle")).fetchCount();
